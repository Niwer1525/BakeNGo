...
public class InsertionManager {
    ...
    private InsertionManager(String name, boolean ignoreIfExists) {
     ...
     */
    public static InsertionManager insert(String tableName) { return new InsertionManager(tableName, false); }

    /**
     * Starts an insertion query for the specified table, ignoring existing entries with the same primary key.
     * @param tableName The name of the table to select from
     */
    public static InsertionManager insertOrIgnore(String tableName) { return new InsertionManager(tableName, true); }
